...
public abstract class MP3TestCase extends TestCase {
    ...
    protected void setUp() throws Exception {
        ...
        defV2 = ("ID3\3\0\0\0\0\0\173" + "TIT2\0\0\0\6\0\0\0"
                + "Title" + // 16
...
/*        for (Enumeration e = testFiles.elements(); e.hasMoreElements();) {
     ...
     * 
     * @param hande
     *            a unique identifier for this test file
     * @param v1
     *            the bytes to append
     * @param v2
     *            the bytes to prepend
     * @param frames
     *            the number of MP3 frames to create
     * @return a File object for accessing the newly created test file
     */
    protected void createMP3(String handle, byte[] v1, byte[] v2,
            int frames) throws FileNotFoundException, IOException {
     ...
     */
    protected void createMP3(String handle, byte[] v1, byte[] v2,
            int frames, byte[] leadin, byte[] regular)
        ...
        for (int i = 0; i < frames - 1; i++) {
            System.arraycopy(regular, 0, content, offset,
                    regular.length);
        ...
        File file = new File(TESTFILE_PATH + File.separator + handle
                + ".mp3");
